                ...
                <form ref={form} onSubmit={submitForm} className='shadow bg-white rounded-2xl space-y-5 px-5 py-3'>
                    ...
                    <div className='flex items-center gap-4 '>
                        <div className=' text-lg '>
                            ...
                            <input name='firstName' value={payload.name} onChange={(e)=>setPayload({ ...payload,name:e.target.value})} type="text" className='px-4 text-md py-2 dark:text-black border border-gray-100 outline-none rounded-md bg-gray-100'  placeholder='Enter your first name'/>
                        </div>
                        <div className=''>
                            <label htmlFor="" className='block dark:text-black text-lg my-2'>Last Name</label>
                            <input name='lastName' value={payload.surName} onChange={(e)=>setPayload({ ...payload,surName:e.target.value})} type="text" className='px-4 border text- dark:text-black border-gray-100 outline-none py-2 rounded-md bg-gray-100'  placeholder='Enter your last name'/>
                        </div>
                    </div>
            
                    <div className=' '>
                        <label htmlFor="" className='block dark:text-black text-lg my-2'>Email</label>
                        <input name='email' value={payload.email} onChange={(e)=>setPayload({ ...payload,email:e.target.value})} type="text" className=' w-full px-4 border text-md dark:text-black border-gray-100 outline-none py-2 rounded-md bg-gray-100'  placeholder='Enter your  email'/>
                    </div>

                     <div className=''>
                        <label htmlFor="" className='block dark:text-black text-lg my-2'>How can we help you ?</label>
                        <textarea name='massage' value={payload.subject} onChange={(e)=>setPayload({ ...payload,subject:e.target.value})} className='border border-gray-200 px-3 py-2 text-md dark:text-black outline-none bg-gray-100 w-full rounded-md h-[100px]' name="" id=""></textarea>
                    </div>
                    <button  className='bg-amber-600 shadow  float-right flex items-center gap-2 rounded-md px-3 py-1 text-lg text-white'>Sent Message
